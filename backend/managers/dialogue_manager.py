...
class Dialogue_Manager(Hok_DB):
    ...
    def get_dialogue_node(self, node_id):
        ...
        for option in options:
            ...
            for evt in events:
                events_data.append(
                    {"event_id": evt[1], "event_type": evt[2], "metadata": evt[3]}
                )
            options_data.append(
                {
                    "option_id": option[1],
    ...
    def get_dialogue_root_nodes(self, npc_id):
        ...
        return {"npc_id": npc_id, "root_node_ids": root_node_ids}

    def get_root_nodes(self, npc_id):
        command = (
            "SELECT node_id FROM dialogue_nodes "
    ...
    def get_next_nodes(self, parent_node_id):
        command = (
            "SELECT * FROM dialogue_nodes WHERE parent_node_id='%s'" % parent_node_id
        )
        return self.db.get_data(command)
    ...
    def create_node(self, node_id, parent_node_id, npc_id):
        self.db.insert(
            "dialogue_nodes",
            [{"node_id": node_id, "parent_node_id": parent_node_id, "npc_id": npc_id}],
        )
    ...
    def delete_node(self, node_id):
        children = self.db.get_data(
            "SELECT node_id FROM dialogue_nodes WHERE parent_node_id = '%s'" % node_id
        )
        if children:
            ...
            [
                {
                    ...
                    "translation_HAN": translation_HAN,
                    "translation_POJ": translation_POJ,
                    "audio_clip": "",
                    "npc_id": npc_id,
                }
            ],
    ...
    def update_dialogue(
        self, node_id, dialogue_text, translation_HAN, translation_POJ, audio_path
    ):
        self.db.update(
            "dialogues",
            {
    ...
    def create_option(
        self, node_id, option_id, option_text, next_node_id, feedback_type
    ):
        self.db.insert(
            "options",
            [
